...
def create_twitter_table_if_not_exist(**kwargs):
    ...
    tweet_table_schema_path = str(
        Path(
            os.getenv(AUX_FILES_DIRECTORY_ENV_NAME),
            'twitter', 'twitter_tweet.schema.json'
        )
    )
    create_twitter_data_pipeline_tables(
        pipeline_conf_list,
        user_table_schema_path=user_table_schema_path,
        tweet_table_schema_path=tweet_table_schema_path
    )
...
with DAG(
    ...
    PythonOperator(
        task_id='create_twitter_data_pipeline_tables',
