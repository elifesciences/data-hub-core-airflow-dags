...
def get_data_config(**kwargs):
    ...
    env_based_data_config = data_config_dict if dep_env is None \
...
def crossref_data_etl(**kwargs):
    ...
    until_collected_date = None
    dataset = data_config.dataset
    table = data_config.table
    externally_triggered_parameters = kwargs['dag_run'].conf
    if externally_triggered_parameters is not None:
        ...
        current_timestamp = temp_current_timestamp \
            if temp_current_timestamp is not None else current_timestamp
        temp_latest_journal_download_date = \
            externally_triggered_parameters.get(ExternalTriggerConfig.LATEST_DOWNLOAD_DATE_PARAM_KEY)
        latest_journal_download_date = temp_latest_journal_download_date \
            if temp_latest_journal_download_date is not None \
            else latest_journal_download_date
        dataset = \
            externally_triggered_parameters.get(
                ExternalTriggerConfig.BQ_DATASET_PARAM_KEY, data_config.dataset)
        table = \
